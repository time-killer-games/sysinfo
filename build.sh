...
if [ $(uname) = "Darwin" ]; then
  ...
  g++ main.cpp system.cpp -o sysinfo -std=c++17 -DCREATE_CONTEXT -static-libgcc -static-libstdc++ `pkg-config --cflags --libs sdl2` `pkg-config --cflags --libs x11` -lGL `pkg-config --cflags --libs hwloc --static` -lpthread; ./sysinfo -h 2> /dev/null;
elif [ $(uname) = "FreeBSD" ]; then
  clang++ main.cpp system.cpp -o sysinfo -std=c++17 -DCREATE_CONTEXT `pkg-config --cflags --libs sdl2` `pkg-config --cflags --libs x11` -lGL `pkg-config --cflags --libs hwloc --static` -lkvm -lpthread; ./sysinfo -h 2> /dev/null;
  ...
  clang++ main.cpp system.cpp -o sysinfo -std=c++17 -DCREATE_CONTEXT `pkg-config --cflags --libs sdl2` `pkg-config --cflags --libs x11` -lGL `pkg-config --cflags --libs hwloc --static` -lpthread; ./sysinfo -h 2> /dev/null;
  ...
  g++ main.cpp system.cpp -o sysinfo.exe -std=c++17 -static-libgcc -static-libstdc++ `pkg-config --cflags --libs hwloc --static` -lws2_32 -ldxgi -lpsapi -static; ./sysinfo -h 2> /dev/null;
fi
