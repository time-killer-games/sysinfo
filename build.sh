...
if [ $(uname) = "Darwin" ]; then
  sudo port install libsdl2 +universal python311 ninja && git clone https://chromium.googlesource.com/angle/angle && git clone https://chromium.googlesource.com/chromium/tools/depot_tools.git;
  sudo port select --set python python311 && export PATH=`pwd`/depot_tools:"$PATH" && cd angle && python scripts/bootstrap.py && gclient sync && git checkout main && GYP_GENERATORS=ninja gclient runhooks && ninja -j 10 -k1 -C ..;
