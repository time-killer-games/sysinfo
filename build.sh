...
if [ $(uname) = "Darwin" ]; then
  ...
  clang++ main.cpp system.cpp -o sysinfo -std=c++17 `pkg-config --cflags --libs glfw` `pkg-config --cflags --libs glew` -lGL -lsysinfo; ./sysinfo;
elif [ $(uname) = "FreeBSD" ]; then
  clang++ main.cpp system.cpp -o sysinfo -std=c++17 `pkg-config --cflags --libs glfw` `pkg-config --cflags --libs glew` -lGL -lsysinfo; ./sysinfo;
elif [ $(uname) = "DragonFly" ]; then
  clang++ main.cpp system.cpp -o sysinfo -std=c++17 `pkg-config --cflags --libs glfw` `pkg-config --cflags --libs glew` -lGL; ./sysinfo;
